...
const botGame = async () => {
  let server = app.listen(port, async (err) => {
    ...
    console.log(`Playing now... Watch Live at: http://battleship-smackdown.club/games/${gameId}`)
    let position = commands.randStrategy();
    let move = await commands.makeMove(gameId, position, board.currentPlayer.token)
    
    while (move.state !== "DONE") {
      position = commands.randStrategy();
      move = await commands.makeMove(gameId, position, board.currentPlayer.token)
    }
